...
export default function ExperiencePage() {
  ...
  const renderToolCards = (tools: Tool[]) => {
    if (!tools || tools.length === 0) {
      return <p className="text-lightSky text-center">No tools available in this category.</p>;
    }
          ...
          <TooltipTrigger asChild aria-label={`Tool: ${tool.name}`}>
            <div className="group basis-1/4 relative p-4 rounded-xl  border-lightSky/30 hover:bg-lightSky/10 cursor-pointer transition-all duration-300">
              <div className="text-center">
                <div className="flex justify-center mb-4 text-lightSky text-2xl">{React.createElement(tool.icon)}</div>
                <h3 className="text-sm md:inline-flex md:text-lg text-white/80">{tool.name}</h3>
              </div>
    ...
    <div className="bg-bodyColor text-white/80">
      <Container className="bg-bodyColor text-white/80 px-4 py-20 md:px-20 md:py-16 min-h-screen content-center">
        ...
        <p className="text-center text-white/70 text-lg md:text-xl mt-4 max-w-4xl mx-auto">
          I have hands-on experience with various frontend, backend, database, and development tools. 
          Over the years, I have honed my skills to design and build efficient, user-friendly applications. 
          I&apos;m familiar with industry-standard practices, modern frameworks, and deployment strategies, 
          which enable me to deliver high-quality solutions.
        </p>
        ...
        {/* Tools Introduction */}
        <h4 className="text-center text-white/70 text-lg md:text-xl mt-8 max-w-3xl mx-auto">
          My toolkit includes a diverse set of technologies to handle end-to-end application development. 
          From crafting intuitive user interfaces to building scalable backends, I am familiar with these tools:
        </h4>

        {/* Frontend Tools */}
        <div className="mt-10">
          <h2 className="text-2xl font-bold text-white mb-4" role="heading" aria-level={2}>
            Frontend
          </h2>
